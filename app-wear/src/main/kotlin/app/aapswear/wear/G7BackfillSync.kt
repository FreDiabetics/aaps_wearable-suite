...
internal object G7BackfillSync {
    suspend fun sendPending(
        ...
        val repository = ProviderG7ReadingRepository(context)
        val manager =
            G7ReadingSyncManager(
                repository,
                WearDataLayerG7SyncTransport(context, targetNodeId),
            )
        return manager.sendPending(G7ReadingBatch.MAX_READINGS)
    }
...
private class ProviderG7ReadingRepository(
    ...
    override suspend fun getRecent(sinceEpochMs: Long): List<CgmReading> =
        query("readings", 300).filter { it.timestampEpochMs >= sinceEpochMs }
    override suspend fun getRange(fromEpochMs: Long, toEpochMs: Long): List<CgmReading> =
        query("readings", 300).filter { it.timestampEpochMs in fromEpochMs..toEpochMs }
            ...
            Intent("app.aapswear.g7watch.ACK_SYNC")
                .setComponent(
                    ComponentName(
                        ...
                        "app.aapswear.g7watch.G7SyncControlReceiver",
                    ),
                )
    ...
    private fun query(path: String, limit: Int): List<CgmReading> =
        runCatching {
            context.contentResolver.query(
                Uri.parse("content://app.aapswear.g7watch.readings/$path"),
                null,
                null,
                null,
                null,
                    ...
                    while (cursor.moveToNext() && size < limit) {
                        add(
                            CgmReading(
                                ...
                                sensorId = cursor.string("sensor_id"),
                                sessionId = cursor.string("session_id"),
                                ...
                                trendRateMgDlPerMinute = cursor.doubleOrNull("trend_rate"),
                                predictedMgDl = cursor.doubleOrNull("predicted"),
                                ...
                                sequenceNumber = cursor.longOrNull("sequence_number"),
                                displayOnly = cursor.intOrNull("display_only") == 1,
    ...
    private fun android.database.Cursor.intOrNull(name: String): Int? =
        getColumnIndexOrThrow(name).let { if (isNull(it)) null else getInt(it) }
}
