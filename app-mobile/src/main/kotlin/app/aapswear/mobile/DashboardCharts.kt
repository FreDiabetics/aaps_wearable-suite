            ...
            MotionEvent.ACTION_MOVE -> {
                if (!scaleDetector.isInProgress && event.pointerCount == 1) {
                    ...
                    val horizontalGesture =
                        abs(event.x - downX) > abs(event.y - downY) + 4f
...
internal class GlucoseDashboardChart @JvmOverloads constructor(
    ...
    fun bind(
        ...
        this.cgmDotRadiusDp = cgmDotRadiusDp.coerceIn(1.5f, 6.0f)
        this.cgmDotOutlineEnabled = cgmDotOutlineEnabled
        this.cgmDotOutlineWidthDp = cgmDotOutlineWidthDp.coerceIn(0.25f, 3.0f)

        if (!isAttachedToWindow) {
            viewport.setHours(durationHours.toFloat())
        }
        invalidate()
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.withClip(clip) {
            ...
            if (showTargetValue) {
                ...
                val targetValueY = mapGlucoseY(targetValue, plot)
                ...
                canvas.drawLine(plot.left, targetValueY, plot.right, targetValueY, linePaint)
                linePaint.pathEffect = null
                drawText(
                    canvas,
                    "Ziel ${glucoseLabel(targetValue)}",
                    plot.right - 1f.dp,
                    (targetValueY - 4f.dp).coerceAtLeast(plot.top + 11f.dp),
                    9f,
                    SugarliciousColors.argb(SugarliciousColorRole.GRAPH_LABEL),
                    Paint.Align.RIGHT,
                )
            ...
            val historyRightEdge =
                if (predictionLaneVisible) dividerX - 2f.dp else plot.right - 4f.dp
    ...
    private fun drawGrid(
        ...
        val zeroY = mapGlucoseY(0.0, plot)
        linePaint.color = withAlpha(
            SugarliciousColors.argb(SugarliciousColorRole.GRAPH_GRID),
            185,
        )
    ...
    private fun drawBasal(
        ...
        val maxBasal = max(
            ...
            visible.flatMap {
                listOfNotNull(
                    it.baseBasalUnitsPerHour ?: it.basalUnitsPerHour,
                    effectiveBasal(it),
                )
            }.maxOrNull() ?: 0.1,
        )
        fun basalY(value: Double): Float =
            plot.top + (value.coerceIn(0.0, maxBasal) / maxBasal).toFloat() * plot.height() * BASAL_HEIGHT_FRACTION
        ...
        canvas.withClip(clip) {
            ...
            linePaint.strokeWidth = 1.2f.dp
            linePaint.pathEffect = null
            canvas.drawPath(stepPath(effective, start, end, plot, ::basalY), linePaint)
            linePaint.strokeWidth = 1f.dp
            linePaint.pathEffect = DashPathEffect(floatArrayOf(1f.dp, 2f.dp), 0f)
            canvas.drawPath(stepPath(base, start, end, plot, ::basalY), linePaint)
            linePaint.pathEffect = null
        }
    }
    ...
    private fun drawInsulinActivity(
        ...
        val future = buildActivityProjection(
            actual.last(),
            max(now, actual.last().first),
            end,
        )
        val maxActivity = max(
            0.0001,
        ...
        fun activityY(value: Double): Float =
            band.bottom -
                (value / maxActivity).coerceIn(0.0, 1.0).toFloat() *
                band.height() * ACTIVITY_HEIGHT_FRACTION
        ...
        linePaint.pathEffect = null
        canvas.drawPath(
            smoothValuePath(smoothedActual, start, end, band, ::activityY),
            linePaint,
        )
        if (smoothedFuture.size >= 2) {
            linePaint.pathEffect = DashPathEffect(floatArrayOf(4f.dp, 4f.dp), 0f)
            canvas.drawPath(
                smoothValuePath(smoothedFuture, start, end, band, ::activityY),
                linePaint,
            )
            linePaint.pathEffect = null
        }
...
internal class MetabolicDashboardChart @JvmOverloads constructor(
    ...
    private fun drawSharedGrid(
        ...
        canvas.drawLine(
            iob.left,
            iob.bottom + (cob.top - iob.bottom) / 2f,
            iob.right,
            iob.bottom + (cob.top - iob.bottom) / 2f,
            linePaint,
        )
    ...
    private fun drawLane(
        ...
        val zeroY = y(0.0)
        val color = SugarliciousColors.argb(
            if (iob) SugarliciousColorRole.GRAPH_IOB else SugarliciousColorRole.GRAPH_COB,
        )
        val area = Path().apply {
        ...
        if (range.minimum < -0.01) {
            drawText(
                canvas,
                formatMetabolicScale(range.minimum),
                labelX,
                plot.bottom - 7f.dp,
                8f,
                Color.WHITE,
    ...
    private fun drawSmbMarkers(
        ...
        val markers = points.mapNotNull { point ->
            point.smbUnits?.takeIf { it > 0.0 }?.let { point.measuredAtEpochMs to it }
        }
...
internal fun toolkitMetabolicRange(
    ...
    val minimum =
        sharedZeroRatio?.let {
            toolkitMinimumForZeroRatio(maximum, it)
        } ?: if (referenceMinimum < 0.0) {
            referenceMinimum * 1.08
        } else {
            -maximum * 0.08
        }
...
internal fun glucoseLogRatio(valueMgDl: Double): Double {
    ...
    return when {
        value <= 80.0 ->
            GLUCOSE_ZERO_RATIO +
                value / 80.0 * (GLUCOSE_LOW_RATIO - GLUCOSE_ZERO_RATIO)

        value <= 160.0 ->
            GLUCOSE_LOW_RATIO +
                (ln(value / 80.0) / ln(2.0)) *
                (GLUCOSE_TARGET_HIGH_RATIO - GLUCOSE_LOW_RATIO)

        else ->
            GLUCOSE_TARGET_HIGH_RATIO +
                (ln(value / 160.0) / ln(GLUCOSE_DISPLAY_MAX / 160.0)) *
                (1.0 - GLUCOSE_TARGET_HIGH_RATIO)
...
private fun windowedStepSamples(
    ...
    if (combined.first().measuredAtEpochMs > start) {
        combined.add(0, combined.first().copy(measuredAtEpochMs = start))
    }
    if (combined.last().measuredAtEpochMs < end) {
        combined += combined.last().copy(measuredAtEpochMs = end)
    }
    return combined
}
...
private fun timeGridIntervalMs(hours: Float): Long =
    when {
...
private fun signedLogTransform(value: Double, linearScale: Double): Double =
    when {
...
private fun mapSignedLogY(
    ...
    val ratio = ((transformedValue - transformedMin) /
        (transformedMax - transformedMin).coerceAtLeast(0.000001)).coerceIn(0.0, 1.0)
...
private fun smoothValuePath(
    ...
    if (values.isEmpty()) return@apply
    val mapped = values.map { (time, value) ->
        mapX(time, start, end, plot) to mapValue(value)
    }
...
private fun buildActivityProjection(
    last: Pair<Long, Double>,
    projectionStart: Long,
    end: Long,
): List<Pair<Long, Double>> {
